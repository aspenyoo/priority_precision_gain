...
% if subject 'GH', need to replace TarY
if strcmp(edf_fn(end-8:end-7),'GH'); 
    
    % figure out which run this is
    irun = str2double(edf_fn(end-5:end-4));
    
    % load designmat and get associated target locations
    load('/Volumes/data/Pri_quad/GH/experiment/GH_designMat.mat');
    targ_loc = designMat((irun-1)*12+(1:12),18:19);
    
    % get indices of target locations
    idx_start = find(diff(ii_data.XDAT==6)==1)+1;
    idx_end = find(diff(ii_data.XDAT==7)==-1);
    
    % these numbers should be the same if this is the correct subject and
    % run
    assert(abs(sum(ii_data.TarX(idx_start) - targ_loc(:,1))) < 1e-2)
    
    for itrial = 1:12;
        ii_data.TarY(idx_start(itrial):idx_end(itrial)) = targ_loc(itrial,2);
    end
end
...
% [ii_data,ii_cfg] = ii_invert(ii_data,ii_cfg,'Y'); % ASPEN: commented this
% out bc y was inverted
